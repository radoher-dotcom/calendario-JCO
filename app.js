...
var html = '<div class="max-w-7xl mx-auto p-4"><div class="bg-dark-card border-glow rounded-xl shadow-2xl p-6 mb-6"><div class="flex justify-between items-center flex-wrap gap-4"><div class="flex items-center gap-4"><img src="https://raw.githubusercontent.com/radoher-dotcom/calendario-JCO/main/JCO%20dorado_070422.png" alt="Logo" class="w-16 h-16 rounded-full glow-gold"><div><h1 class="text-3xl font-bold text-gold glow-gold-strong retro-title">CALENDARIO</h1><p class="text-gray-300 mt-1"><span class="font-semibold text-white">' + currentUser.name + '</span><span class="ml-2 text-sm ' + (isAdmin ? 'bg-yellow-600 text-black' : 'bg-gradient-to-r ' + myRank.color + ' text-gray-900') + ' px-3 py-1 rounded-full font-bold">' + (isAdmin ? 'ADMIN' : myRank.emoji + ' ' + myRank.name) + '</span></p></div></div><button onclick="logout()" class="bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white px-4 py-2 rounded-lg transition">Salir</button></div></div>';
